        ...
        img.addEventListener('click', () => {
            arr.forEach(hero => {
                if (img.getAttribute('src').match(hero.name)) {
                    ...
                    <p>
                        Имя: ${hero.name} <br>
                        Количество умений: ${hero.numberOfSkills} <br>
                        Сумма уровней всех навыков: ${hero.sumLevels} <br>
                        Ключевой навык: ${hero.mainSkillName} <br>
                        Количество комиксов с героем: ${hero.numberOfComics} <br>
                        Названия комиксов: ${hero.titleOfComics} 
                    </p>
                    `;
                }
            });
